...
#----------------------------------------------------------------------------------
def apply_mge(cut_data, level, minlevel, fwhm, Ar, skylev = 0, scale = 0.396, ngauss = 12, Msolar = 4.65,
              ...
              psf_label_x = None, psf_label_y = None, extra_minlevel = 0, extra_mask = None,
              compass = False, xc = 0, yc = 0, pa = 0, length = 0, N_x_pad = 1, N_y_pad = 1, E_x_pad = 1, E_y_pad = 1):
    '''
    ...
    - Msolar: absolute magnitude of the sun (e.g., 4.65 for SDSS r band (Willmer 2018)).
    ...
    if compass:
        add_NE_compass(ax = ax, xc = xc, yc = yc, pa_deg = pa, length = length,
                       N_x_pad = N_x_pad, N_y_pad = N_y_pad, E_x_pad = E_x_pad, E_y_pad = E_y_pad)
...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(2, 5, figsize = (50/3, 6))
    ...
    fits_path = '300787/MGE/300787_frame-r.fits'
    ...
    output_path = '300787/MGE/300787_cut_image_200_arc.fits'
    ...
    fits_path = '300787/MGE/segmentation.fits'
    mask_map = mask_map_create(fits_path, target_label = 10)
    # ---------------------------------------------------------------------------------

    # third step: apply the MGE.
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.1, minlevel = 0.1,
              fwhm = 3.22295202, ngauss = 12, Ar = 0.08618277, twist = False,
    ...
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.1, minlevel = 0.1,
              ...
              Re = 2.3921091557, psf_label_x = -6.5, psf_label_y = -6.5)
    ...
    cut_data = image_cutout(fits_path = fits_path, ra = ra, dec = dec, scale = scale, cut_size = cut_size,
                            output_path = output_path, vmin = 0, vmax = 2, rotation = True, align_major = True,
                            counterclock = False, IPA = True, kin_pa = 70.15)
    # --------------------------------------------------------------------------------
    # step two: prepare the mask map.
    ...
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.1, minlevel = 0.1,
              ...
              N_x_pad = 0.22, N_y_pad = 0.09, E_x_pad = -0.62, E_y_pad = 0.1)
    ...
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.11, minlevel = 0.11,
              ...
              N_x_pad = -0.8, N_y_pad = 1.2, E_x_pad = -0.2, E_y_pad = -0.6, extra_mask = r_mask)

    #-----------------------------------------------------------------------------------
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.11, minlevel = 0.11,
              fwhm = 2.828, ngauss = 12, Ar = 0.08768186, twist = False, ax = axs[1, 2],
    ...
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.15, minlevel = 0.15,
              fwhm = 3.31633, ngauss = 12, Ar = 0.06149926, twist = False, ax = axs[0, 3],
              tick_lim = 80, loc_min = -80, loc_max = 85, loc_step = 40, title = 'Galaxy 230776',
              Re = 23.8104171753, psf_label_x = -70, psf_label_y = -70,
              compass = True, xc = -50, yc = 50, length = 20, pa = -11.3,
              N_x_pad = 0, N_y_pad = 2.0, E_x_pad = -2.0, E_y_pad = -1.0)

    # zoom in.
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.15, minlevel = 0.15,
              fwhm = 3.31633, ngauss = 12, Ar = 0.06149926, twist = False, ax = axs[1, 3],
              tick_lim = 7.5, loc_min = -7.5, loc_max = 7.6, loc_step = 2.5,
              Re = 23.8104171753, psf_label_x = -6.5, psf_label_y = -6.5)
    ...
    # usage example for SDSS r band image and sextractor.
    # step one: cutout the image and estimate PSF (PSF can also be extracted from the catalog).
    fits_path = '143287/MGE/143287_frame-r.fits'
    ...
    # --------------------------------------------------------------------------------
    # step two: prepare the mask map.
    fits_path = '143287/MGE/segmentation.fits'
    mask_map = mask_map_create(fits_path, target_label = 6)
    # ---------------------------------------------------------------------------------
    # sectractor is unable to extract the nearby galaxy, so we mask it manually.
    extra_mask = make_circular_extra_mask(cut_data.shape, 101, 124, 3)
    ...
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.07, minlevel = 0.07,
              ...
              Re = 3.1987638474, psf_label_x = -6.5, psf_label_y = -6.5, extra_mask = extra_mask)
    ...
    # usage example for SDSS r band image and sextractor.
    # step one: cutout the image and estimate PSF (PSF can also be extracted from the catalog).
    fits_path = '9239900248/MGE/9239900248_frame-r.fits'
    ...
    apply_mge(cut_data = cut_data, mask_map = mask_map, level = 0.1, minlevel = 0.1,
              ...
              ax = axs[0, 5], tick_lim = 16, loc_min = -16, loc_max = 17, loc_step = 8,
              title = 'Galaxy 9239900248', Re = 2.3038685322, psf_label_x = -14, psf_label_y = -14,
              compass = True, xc = -10, yc = 10, length = 4, pa = 56.583,
              N_x_pad = -0.45, N_y_pad = 0.43, E_x_pad = -0.02, E_y_pad = -0.2)
