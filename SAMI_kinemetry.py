...
pa_and_k1_plot(k_143287, axs = axs[:, 1], ypa_lim = [-200, 75], ypa_tick = (-200, 76, 50), yk1_lim = [0, 110], yk1_tick = (0, 110, 20),
...
k_227266 = kinemetry(xbin = xbin, ybin = ybin, moment = velbin, error = er_velbin, x0 = np.median(xbin), y0 = np.median(ybin), rangeQ = [0.79, 0.99],
                     rangePA = [0, 200], npa = 41, nq = 41, plot = False, scale = 1, ring = 0.5, cover = 0.9)
...
pa_and_k1_plot(k_227266, axs = axs[:, 2], ypa_lim = [0, 200], ypa_tick = (0, 205, 50), yk1_lim = [0, 25], yk1_tick = (0, 25, 5),
               x_lim = [0, 7.5], x_tick = (0, 7.5, 1), pa1 = np.mean(k_227266.pa[:5]), pa2 = np.mean(k_227266.pa[-6:]),
               pa1_err = np.std(k_227266.pa[:5]), pa2_err = np.std(k_227266.pa[-6:]), label_pad = 0)
...
# by using the fit_kinematic_pa code, we estimate the pa in the CRC to be 38.0 +/- 15.2 degrees and the pa for the main stellar body to be 34.5 +/- 36.8 degrees.
k_9239900248 = kinemetry(xbin = xbin, ybin = ybin, moment = velbin, error = er_velbin, x0 = np.median(xbin), y0 = np.median(ybin), rangeQ = [0.47, 0.67],
                         rangePA = [-160, -130], npa = 41, nq = 41, plot = True, scale = 1, ring = 0.2, cover = 0.20)
...
k_9239900248_extra = kinemetry(xbin = xbin, ybin = ybin, moment = velbin, error = er_velbin, x0 = np.median(xbin), y0 = np.median(ybin), rangeQ = [0.47, 0.67],
                         rangePA = [-160, -130], npa = 41, nq = 41, plot = False, scale = 1, ring = 0.5, cover = 0.20)
...
kdc_143287 = [50, 50, 46.51527849, 26]
print('kdc 143287:', np.mean(kdc_143287), np.std(kdc_143287))

main_143287 = [-165.54784139, -163, -156.74627493, -163.43865521, -159.94165077]
print('main 143287:', np.mean(main_143287), np.std(main_143287))

kdc_227266 = [182.54172534, 167.38768068, 172.75838868, 150, 180]
print('kdc 227266:', np.mean(kdc_227266), np.std(kdc_227266))

main_227266 = [33.51363259, 34.63677148, 25, 35.49163644, 39.59152783, 34.62241028, 24.15998526]
print('main 227266:', np.mean(main_227266), np.std(main_227266))

kdc_230776 = [4.33772754, 1.41765148, 2.7452081, 2.05103039, 6.76379129, 13.5]
print('kdc 230776:', np.mean(kdc_230776), np.std(kdc_230776))

main_230776 = [-58.2173712, -81, -72.66392056, -80.96039109, -75.76672103, -77.69839479]
print('main 230776:', np.mean(main_230776), np.std(main_230776))

kdc_9239900248 = [49.88426435, 44.46076784, 45.5, 50, 20]
print('kdc 9239900248:', np.mean(kdc_9239900248) - 180, np.std(kdc_9239900248))

main_9239900248 = [-160, -158.5, -155.5, -155.5, -142.78918547]
print('main 9239900248:', 180 + np.mean(main_9239900248), np.std(main_9239900248))