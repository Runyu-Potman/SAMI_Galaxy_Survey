...
def plot_vel_or_sig(csv_path, cmap = 'RdBu_r', cbar_label = 'km/s', value_type = 'vel',
                    ...
                    pa_center_x = 0, pa_center_y = 0, plot_psf = False, psffwhm = 2, text_ul = None, galaxy_name = None):
    """
    ...
    if PAs is not None:
        ...
        for PA, ll, ls in zip(PAs, lengths, line_styles):
            ...
            ax.plot([x1, x2], [y1, y2], color = 'limegreen', lw = 2, linestyle = ls)
...
if __name__ == '__main__':
    ...
    # ------------------------------------------------------------------------------------------------
    # 300787 stellar kinematic files.
    star_vel_300787 = '300787/kinematic/300787_A_stellar-velocity_default_two-moment.fits'
    star_sig_300787 = '300787/kinematic/300787_A_stellar-velocity-dispersion_default_two-moment.fits'
    star_output_file_300787 = '300787/kinematic/300787_quality_cut_stellar_velocity_map.csv'

    # 300787 stellar quality cut.
    quality_cut_stellar_velocity_map_csv(star_vel_300787, star_sig_300787, star_output_file_300787, pixel_to_arc = True)
    #-------------------------------------------------------------------------------------------------
    ...
    vel_cmap_143287 = cmr.get_sub_cmap('twilight_shifted', 0, 0.93)
    vel_cmap_230776 = cmr.get_sub_cmap('twilight_shifted', 0, 0.68)
    vel_cmap_300787 = cmr.get_sub_cmap('twilight_shifted', 0.1, 1.0)
    vel_cmap_9239900248 = cmr.get_sub_cmap('twilight_shifted', 0.15, 0.95)

    sig_cmap = cmr.get_sub_cmap('twilight_shifted', 0.05, 0.6)
    sig_cmap_143287 = cmr.get_sub_cmap('twilight_shifted', 0.05, 0.7)
    sig_cmap_9239900248 = cmr.get_sub_cmap('twilight_shifted', 0, 0.68)
    # ------------------------------------------------------------------------------------------
    # 7969 optical.
    optical_7969 = Image.open('7969/optical/7969_optical_image.jpg')
    axs[0, 0].imshow(optical_7969)
    axs[0, 0].set_ylabel('Galaxy 7969', fontsize = 12, labelpad = 8)
    add_circle(axs[0, 0], optical_7969, linewidth = 1, label = True, E_bar = 1.6)
    axs[0, 0].set_title('SDSS Optical Image', fontsize = 12, pad = 8)

    # 7969 kinematics.
    plot_vel_or_sig(csv_path = star_output_file_7969, value_type = 'vel', ax = axs[0, 1], cmap = vel_cmap_7969, cbar_label= r'$V_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 12, psffwhm = 1.561, vmin = -80, vmax = 80)
    plot_vel_or_sig(csv_path = star_output_file_7969, value_type = 'sig', ax = axs[0, 2], cmap = sig_cmap, cbar_label = r'$\sigma_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 12, psffwhm = 1.561, vmin = 145, vmax = 305)
    axs[0, 1].set_title('Stellar Velocity', fontsize = 12, pad = 8)
    axs[0, 2].set_title('Stellar Velocity Dispersion', fontsize = 12, pad = 8)

    # ------------------------------------------------------------------------------------------
    # 143287 optical.
    optical_143287 = Image.open('143287/optical/143287_optical_image.jpg')
    axs[1, 0].imshow(optical_143287)
    axs[1, 0].set_ylabel('Galaxy 143287', fontsize = 12, labelpad = 8)
    add_circle(axs[1, 0], optical_143287, linewidth = 1, label = True, E_bar = 1.5)

    # 143287 kinematics.
    plot_vel_or_sig(csv_path = star_output_file_143287, value_type = 'vel', ax = axs[1, 1], cmap = vel_cmap_143287, cbar_label = r'$V_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 12, psffwhm = 2.250)
    plot_vel_or_sig(csv_path = star_output_file_143287, value_type = 'sig', ax = axs[1, 2], cmap = sig_cmap_143287, cbar_label = r'$\sigma_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 12, psffwhm = 2.250, vmin = 55, vmax = 185)
    #--------------------------------------------------------------------------------------------
    # 227266 optical.
    optical_227266 = Image.open('227266/optical/227266_optical_image.jpg')
    axs[2, 0].imshow(optical_227266)
    axs[2, 0].set_ylabel('Galaxy 227266', fontsize = 10, labelpad = 8)
    add_circle(axs[2, 0], optical_227266, linewidth = 1, label = True, E_bar = 3.3)

    # 227266 kinematics.
    plot_vel_or_sig(csv_path = star_output_file_227266, value_type = 'vel', ax = axs[2, 1], cmap = vel_cmap, cbar_label = r'$V_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 2.108, vmin = -55, vmax = 55)
    plot_vel_or_sig(csv_path = star_output_file_227266, value_type = 'sig', ax = axs[2, 2], cmap = sig_cmap, cbar_label = r'$\sigma_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 2.108, vmin = 110, vmax = 230)
    # ------------------------------------------------------------------------------------------
    # 230776 optical.
    ...
    # 230776 kinematics.
    plot_vel_or_sig(csv_path = star_output_file_230776, value_type = 'vel', ax = axs[0, 4], cmap = vel_cmap_230776, cbar_label = r'$V_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 2.144, vmin = -35, vmax = 35)
    plot_vel_or_sig(csv_path = star_output_file_230776, value_type = 'sig', ax = axs[0, 5], cmap = sig_cmap, cbar_label = r'$\sigma_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 2.144, vmin = 270, vmax = 350)
    axs[0, 4].set_title('Stellar Velocity', fontsize = 10, pad = 8)
    axs[0, 5].set_title('Stellar Velocity Dispersion', fontsize = 10, pad = 8)
    # --------------------------------------------------------------------------------------------
    # 300787 optical.
    optical_300787 = Image.open('300787/optical/300787_optical_image.jpg')
    axs[1, 3].imshow(optical_300787)
    axs[1, 3].set_ylabel('Galaxy 300787', fontsize = 10, labelpad = 8)
    add_circle(axs[1, 3], optical_300787, linewidth = 1, label = True, E_bar = 1.7)

    # 300787 kinematics.
    plot_vel_or_sig(csv_path = star_output_file_300787, value_type = 'vel', ax = axs[1, 4], cmap = vel_cmap_300787, cbar_label = r'$V_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 1.941, vmin = -130)
    plot_vel_or_sig(csv_path = star_output_file_300787, value_type = 'sig', ax = axs[1, 5], cmap = sig_cmap, cbar_label = r'$\sigma_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 1.941)
    #---------------------------------------------------------------------------------------------
    # 9239900248 optical.
    optical_9239900248 = Image.open('9239900248/optical/9239900248_optical_image.jpg')
    axs[2, 3].imshow(optical_9239900248)
    axs[2, 3].set_ylabel('Galaxy 9239900248', fontsize = 10, labelpad = 8)
    add_circle(axs[2, 3], optical_9239900248, linewidth = 1, label = True, E_bar = 1.7)

    # 9239900248 kinematics.
    plot_vel_or_sig(csv_path = star_output_file_9239900248, value_type = 'vel', ax = axs[2, 4], cmap = vel_cmap_9239900248, cbar_label = r'$V_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 2.118, vmin = -200, vmax = 200)
    plot_vel_or_sig(csv_path = star_output_file_9239900248, value_type = 'sig', ax = axs[2, 5], cmap = sig_cmap_9239900248, cbar_label = r'$\sigma_{\bigstar}$ (km/s)', plot_psf = True, fontsize = 10, psffwhm = 2.118, vmin = 45, vmax = 205)
    #---------------------------------------------------------------------------------------------
    # same axis ratio.
