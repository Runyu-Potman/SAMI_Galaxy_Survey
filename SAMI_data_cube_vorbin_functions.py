...
def vorbin_pre_cube_combine(blue_cube_fits, red_cube_fits, output_filename, fwhm_blue = 2.65, fwhm_red = 1.61):
    ...
    with fits.open(red_cube_fits) as red_hdul:
        red_flux_cube = red_hdul[0].data
        red_var_cube = red_hdul[1].data
        red_header = red_hdul[0].header

    # data cube shape for later use.
    blue_cube_shape_wave = blue_flux_cube.shape[0]
    blue_cube_shape_y = blue_flux_cube.shape[1]
    blue_cube_shape_x = blue_flux_cube.shape[2]
    # debug.
    ...
    # to build a kernel, we need to know how many red pixels that corresponds to, so divide by the red cdelt3.
    ...
    # store the data and new wavelength information.
...
if __name__ == "__main__":
    ...
    #-----------------------------------------------------------------------
    # first step: combine blue and red in preparation for vorbin.
    blue_cube_fits = '300787/age_z/300787_A_cube_blue.fits'
    red_cube_fits = '300787/age_z/300787_A_cube_red.fits'
    output_filename = '300787/age_z/300787_A_cube_combined_pre_vorbin.fits'
    vorbin_pre_cube_combine(blue_cube_fits, red_cube_fits, output_filename)

    #-------------------------------------------------------------------
    # second step: apply vorbin to reach target s/n.
    fits_path = '300787/age_z/300787_A_cube_combined_pre_vorbin.fits'
    output_filename = '300787/age_z/300787_A_cube_combined_vorbin_20.fits'

    sn_threshold = 3
    target_sn = 20.5 # S/N per_angstrom = 20, S/N = 20.5
    cleaned_data_cube, binNum, x_gen, y_gen, x_bar, y_bar, sn, nPixels, scale = data_cube_clean_snr(
        fits_path = fits_path, sn_threshold = sn_threshold, output_filename = output_filename,
        vorbin = True, target_sn = target_sn, wavelength_slice_index = 1024)

    np.save('300787/age_z/binNum.npy', binNum)
    np.save('300787/age_z/x_bar.npy', x_bar)
    np.save('300787/age_z/y_bar.npy', y_bar)
