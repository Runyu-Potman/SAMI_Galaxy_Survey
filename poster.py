...
#-----------------------------------------------------------------------------------
def plot_age_and_Z(axs, axs_x, age_full, metal_full, r_all, age_array, metal_array, age_std_array, metal_std_array,
    ...
    axs[axs_x].set_xlabel('Offset (arcsec)', fontsize = fontsize, labelpad = 8)
    axs[axs_x].set_ylabel('Offset (arcsec)', fontsize = fontsize, labelpad = label_pad)

    # set color bar (first column).
    cbar = plt.colorbar(im, ax = axs[axs_x], fraction = bar_fraction, pad = bar_pad)
    cbar.ax.yaxis.set_tick_params(direction = 'in')
    cbar.set_label('Age (Gyr)', fontsize = fontsize, labelpad = 4)

    # set titles (first column).
    if title:
        axs[axs_x].set_title('Resolved Stellar Age', fontsize = fontsize)

    # make these (major) ticks longer.
    axs[axs_x].tick_params(axis = 'both', which = 'major', length = 4, width = 1, direction = 'in')

    # add minor ticks (shorter, no labels).
    axs[axs_x].xaxis.set_minor_locator(AutoMinorLocator())
    axs[axs_x].yaxis.set_minor_locator(AutoMinorLocator())
    axs[axs_x].tick_params(axis = 'both', which = 'minor', length = 2, width = 1, direction = 'in')
...
if '__main__' == __name__:
    ...
    fig.suptitle('Galaxy 300787 Which Hosts a Counter-rotating Disk', fontsize=15)
