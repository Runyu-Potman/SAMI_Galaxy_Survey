...
version_07: 19/03/2026: replace the clip value with a input parameter. Also for high redshift galaxy, directly using wave_clip would be fine,
                        no need to use convolve anymore.
'''
#-------------------------------------------------------------------------------
...
def ppxf_pre_spectrum(cube_fits, spectrum_fits, high_redshift = False, save_fits = False,
                      convolve = False, wave_clip = False, wave_clip_value = 34, miles_low = 3540.5, miles_high = 7409.6):
    '''
...
def ppxf_pre_data_cube(
        ...
        convolve = False, wave_clip = False, wave_clip_value = 34, miles_low = 3540.5, miles_high = 7409.6, fwhm_blue = 2.65, fwhm_red = 1.61):
    '''
    ...
    for x in range(50):
        for y in range(50):
            ...
            goodpixels_nan, specNew, ln_lam, velscale, redshift = ppxf_pre_data_cube(
                blue_spectrum, blue_fits_path, plot = True, convolve = False, wave_clip = True, wave_clip_value = 34
            )
