...
def bpt(
    ...
    axs[1, 1].text(-0.4, 0.8, 'AGN', color = 'purple', fontsize = fontsize, ha = 'center')

    # set axis labels, title, and limits.
    axs[1, 1].set_xlabel(r'log([SII]/H$\alpha$)', fontsize = fontsize, labelpad = labelpad_x)
    axs[1, 1].set_ylabel(r'log([OIII]/H$\beta$)', fontsize = fontsize, labelpad = labelpad_y)
    ...
    axs[1, 2].set_ylabel(r'log([OIII]/H$\beta$)', fontsize = 10)
    ...
    cbar = fig.colorbar(sc, ax = axs[1, 2], location = 'right', pad = 0.04)
    cbar.set_label('Radius (arcsec)', fontsize = 10)
    cbar.ax.yaxis.set_tick_params(direction = 'in')
    '''
