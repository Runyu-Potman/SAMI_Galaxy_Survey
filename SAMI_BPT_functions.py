...
def gas_distribution(gas_fits_path, output_file = None, threshold = None, dust_correction = False, dust_fits = None,
                     csv = True, x_center = 24.5, y_center = 24.5, scale = 0.5, log_flux = False):
    '''
...
#-----------------------------------------------------------------------------------------------------------------
def bpt_plot(Ha_map_clean, ax, log_x, log_y, center_x = 24.5, center_y = 24.5, scale = 0.5):
    '''
    ...
    sc = ax.scatter(log_x, log_y, c = distance, cmap = 'coolwarm',
...
def bpt(
    ...
    # plot two boundaries.
    axs[1, 0].plot(boundary_1_range, boundary_1_values, color = 'black', linestyle = '-', linewidth = 1, label = 'Ke01')
    axs[1, 0].plot(boundary_2_range, boundary_2_values, color = 'black', linestyle = '--', linewidth = 1, label = 'Ka03')

    # add labels for each region, ha = 'center' centers the text horizontally at the specified x position.
    axs[1, 0].text(-0.8, -0.8, 'HII', color = 'grey', fontsize = fontsize, ha = 'center')
    axs[1, 0].text(0.0, -1, 'Comp', color = 'salmon', rotation = 90, fontsize = fontsize, ha = 'center')
    axs[1, 0].text(0.0, 0.785, 'AGN', color = 'purple', fontsize = fontsize, ha = 'center')
    ...
    cbar = fig.colorbar(sc, ax = axs[1, 0], fraction = bar_fraction, pad = cbar_pad)
    ...
    axs[1, 1].text(-0.3, 0.785, 'AGN', color = 'purple', fontsize = fontsize, ha = 'center')
    ...
    cbar.ax.text(1.6, 0.5, 'HII', va = 'center', ha = 'left', rotation = 90, fontsize = fontsize, color = 'black')
    cbar.ax.text(1.6, 1.5, 'LINER', va = 'center', ha = 'left', rotation = 90, fontsize = fontsize, color = 'black')
    cbar.ax.text(1.6, 2.5, 'AGN', va = 'center', ha = 'left', rotation = 90, fontsize = fontsize, color = 'black')
...
if __name__ == "__main__":
    ...
    star_output_file_143287_kinematics = '143287/kinematic/143287_quality_cut_stellar_velocity_map.csv'
    ...
    # 143287 plotting.
    plot_vel_or_sig(csv_path = gas_output_file_143287_distribution, cmap = 'magma', cbar_label = r"log(Flux/$10^{-16}$erg s$^{-1}$cm$^{-2}$)",
                    value_type = 'gas', ax = axs[0, 0], plot_psf = True, psffwhm = 2.250, vmin = -1.85, vmax = -0.4, galaxy_name = '143287')

    plot_vel_or_sig(csv_path = gas_output_file_143287_kinematics, cmap = vel_cmap_143287, cbar_label = r'$V_\mathrm{gas}$ (km s$^{-1}$)',
                    value_type = 'vel', ax = axs[0, 1], PAs = [35], line_length = 10, plot_psf = True, psffwhm = 2.250, vmin = -170, vmax = 170)

    plot_vel_or_sig(csv_path = gas_output_file_143287_kinematics, cmap = sig_cmap, cbar_label = r'$\sigma_\mathrm{gas}$ (km s$^{-1}$)',
                    value_type = 'sig', ax = axs[0, 2], plot_psf = True, psffwhm = 2.250, vmin = 10, vmax = 130)

    # directly copied from stellar kinematic.
    plot_vel_or_sig(csv_path = star_output_file_143287_kinematics, value_type = 'vel', ax = axs[0, 3], cmap = vel_cmap_143287_star, cbar_label = r'$V_{\bigstar}$ (km s$^{-1}$)',
                    plot_psf = True, fontsize = 12, psffwhm = 2.250, PAs = [44.72, -160.84], line_length = [2, 8.5])
    ...
    plot_vel_or_sig(csv_path = gas_output_file_227266_kinematics, cmap = sig_cmap, cbar_label = r'$\sigma_\mathrm{gas}$ (km s$^{-1}$)',
                    value_type = 'sig', ax = axs[1, 2], plot_psf = True, psffwhm = 2.108)

    # directly copied from stellar kinematics.
    plot_vel_or_sig(csv_path = star_output_file_227266_kinematics, value_type = 'vel', ax = axs[1, 3], cmap = vel_cmap, cbar_label = r'$V_{\bigstar}$ (km s$^{-1}$)',
                    plot_psf = True, fontsize = 12, psffwhm = 2.108, vmin = -55, vmax = 55, PAs = [171.75, 34.53], line_length = [3, 10], pa_center_x = 0.75, pa_center_y = 0.75)
    ...
    # directly copied from stellar kinematic.
    plot_vel_or_sig(csv_path = star_output_file_300787_kinematics, value_type = 'vel', ax = axs[2, 3], cmap = vel_cmap_300787_star, cbar_label = r'$V_{\bigstar}$ (km s$^{-1}$)',
                    plot_psf = True, fontsize = 12, psffwhm = 1.941, vmin = -120, vmax = 120, PAs = [149.12, -53.95], line_length = [2, 7.5])
    ...
    axs[2, 3].text(-2, 5.5, r'$\mathrm{PA}_\bigstar$=$-53.95^\circ$', color = 'black', fontsize = 10, ha = 'left')
    ...
    plt.tight_layout(h_pad = 0, w_pad = 0.85)
